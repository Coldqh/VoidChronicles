...
import { liveSpaceAccessForCivilization } from './development';
...
function settlementPressure(settlement: SettlementState): number {
  return (
    settlement.unrest * 0.45 +
    (100 - settlement.housing) * 0.25 +
    (100 - settlement.health) * 0.2 +
    settlementShortages(settlement).length * 12
  );
}

function destinationScore(settlement: SettlementState): number {
  return (
    settlement.housing * 0.35 +
    settlement.health * 0.25 +
    settlement.security * 0.2 +
    settlement.infrastructure * 0.2 -
    settlement.unrest * 0.35
  );
}

function movePopulationGroups(
  ...
  const groups = Object.values(state.populationGroups).filter(
    (group) => group.settlementId === origin.id && group.population > 0
  );
  ...
  for (const group of groups) {
    const moved = Math.min(
      group.population,
      Math.round((amount * group.population) / total)
    );
    if (moved <= 0) continue;
    state.populationGroups[group.id] = {
      ...group,
      population: group.population - moved,
      migrationDesire: clamp(group.migrationDesire - 12)
    };
    ...
    const next: PopulationGroupState = existing
      ? {
          ...
          health: Math.round(
            (existing.health * existing.population + group.health * moved) /
              Math.max(1, existing.population + moved)
          ),
...
function scheduleForNewColony(
  ...
  if (!events.some((entry) => entry.id === `migration-cycle:${civilizationId}`)) {
    events.push({
...
export function simulateMigrationCycle(
  ...
  if (
    destination &&
    pressure >= 48 &&
    destinationScore(destination) >= destinationScore(origin) + 8
  ) {
    const amount = Math.max(
      50,
      Math.min(
        Math.round(origin.population * 0.035),
        Math.round(destination.population * 0.12 + 5_000)
      )
    );
    state.settlements[origin.id] = {
      ...origin,
      population: Math.max(0, origin.population - amount),
      unrest: clamp(origin.unrest - 4),
      housing: clamp(origin.housing + 2),
      lastUpdatedHour: atHour
    };
    state.settlements[destination.id] = {
      ...destination,
      population: destination.population + amount,
      housing: clamp(destination.housing - 3),
      unrest: clamp(destination.unrest + 2),
      lastUpdatedHour: atHour
    };
    ...
    return {
      ...
      event: {
        kind: 'migration',
        title: `${civilization.name}: переселение между поселениями`,
        ...
        factionIds: Array.from(
          new Set(
            [origin.ownerFactionId, destination.ownerFactionId].filter(
  ...
  const sourceSystem = context.galaxy.systems.find(
    (entry) => entry.id === origin.systemId
  );
  const candidateIds = Array.from(
    new Set([...(sourceSystem?.neighbors ?? []), ...civilization.controlledSystems])
  );
  const candidates = candidateIds
    .map((id) => context.galaxy.systems.find((entry) => entry.id === id))
    .filter(
      (entry): entry is NonNullable<typeof entry> =>
        entry !== undefined && !occupiedSystems.has(entry.id)
    )
    .filter((entry) => entry.danger !== 'extreme')
    .sort((a, b) => {
      const aHabitability = Math.max(
        0,
        ...a.planets.map((planet) => planet.habitability)
      );
      const bHabitability = Math.max(
        0,
        ...b.planets.map((planet) => planet.habitability)
      );
      return bHabitability - aHabitability;
    });

  const liveSpaceAccess = liveSpaceAccessForCivilization(state, civilization);
  const canColonizeStars =
    liveSpaceAccess === 'interstellar' || liveSpaceAccess === 'ftl';
  if (
    canColonizeStars &&
    civilizationState.expansionPressure >= 68 &&
    origin.population >= 25_000 &&
    candidates.length &&
    rng.chance(0.42)
  ) {
    ...
    state.settlements[origin.id] = {
      ...origin,
      population: Math.max(0, origin.population - colonists),
      lastUpdatedHour: atHour
    };
    ...
    return {
      scheduledEvents: scheduleForNewColony(
      ...
      event: {
        ...
        civilizationIds: [civilizationId],
        factionIds: created.settlement.ownerFactionId
          ? [created.settlement.ownerFactionId]
          : [],
        tags: ['simulation', 'migration', 'colony-founded', 'living-history'],
        data: {
          settlementId: created.settlement.id,
          sourceSettlementId: origin.id,
          population: colonists,
          spaceAccess: liveSpaceAccess
        }
      }
    };
  }
