...
import { simulateCivilizationDevelopmentCycle } from './development';
...
function populationForCivilization(
  context: SimulationContext,
  civilizationId: string
): number {
  const civilization = context.galaxy.civilizations.find(
    (entry) => entry.id === civilizationId
  );
  if (!civilization || civilization.status === 'dead') return 0;

  const deepTimePopulation = civilization.development?.population;
  if (deepTimePopulation !== undefined && Number.isFinite(deepTimePopulation)) {
    return Math.max(0, Math.round(deepTimePopulation));
  }

  const hubPopulation = context.hubs
    .filter((hub) => hub.civilizationId === civilization.id)
    .reduce((sum, hub) => sum + hub.population, 0);
  const territorialBase = civilization.controlledSystems.length * 1_800_000;
...
function systemPopulation(
  ...
  if (!system) return 0;
  const hubPopulation = context.hubs
    .filter((hub) => hub.systemId === systemId)
    .reduce((sum, hub) => sum + hub.population, 0);
  const civilizationPopulation = system.civilizationIds.reduce((sum, id) => {
    ...
    if (!state || !civilization || civilization.controlledSystems.length === 0) return sum;
    return sum + state.population / civilization.controlledSystems.length;
  }, 0);
...
function createBaseSimulation(
  ...
  SimulationState,
  'scheduledEvents' | 'settlements' | 'populationGroups' | 'tradeRoutes'
> {
  const civilizations: Record<string, SimulationCivilizationState> = {};
  for (const civilization of context.galaxy.civilizations) {
    const development = civilization.development;
    const alive = civilization.status === 'living';
    civilizations[civilization.id] = {
      civilizationId: civilization.id,
      population: populationForCivilization(context, civilization.id),
      stability: alive
        ? clamp(development?.stability ?? 46 + civilization.techLevel * 4)
        : 0,
      economy: alive
        ? clamp(
            30 +
              civilization.techLevel * 4 +
              (development?.industrialization ?? 0) * 0.18
          )
        : 0,
      military: alive ? clamp(24 + civilization.techLevel * 6) : 0,
      research: alive
        ? clamp(
            22 +
              civilization.techLevel * 5 +
              (development?.innovation ?? 0) * 0.22 +
              (development?.literacy ?? 0) * 0.12
          )
        : 0,
      cohesion: alive ? clamp(42 + (development?.stability ?? 50) * 0.25) : 0,
      expansionPressure: alive
        ? clamp(
            10 +
              civilization.controlledSystems.length * 4 +
              (development?.urbanization ?? 0) * 0.12
          )
        : 0,
      alive,
  ...
  for (const system of context.galaxy.systems) {
    ...
    systems[system.id] = {
      ...
      prosperity: clamp(
        (hub ? 58 : 24) +
          system.civilizationIds.length * 8 -
          (system.danger === 'danger' ? 14 : system.danger === 'extreme' ? 28 : 0)
      ),
      security: clamp(
        hub?.safety === 'safe'
          ? 76
  ...
  for (const faction of context.factions) {
    factions[faction.id] = {
      ...
      tension: clamp(
        faction.enemies.length * 18 + (faction.disposition === 'hostile' ? 28 : 0)
      ),
      lastUpdatedHour: absoluteHour
    };
  }
...
export function initializeSimulation(
  ...
  const layer = initializeContinuitySettlementLayer(
    context,
    base.ecosystems,
    absoluteHour
  );
  const simulation: SimulationState = {
    ...
    scheduledEvents: initialScheduledEvents({
      ...
      civilizationIds: context.galaxy.civilizations
        .filter((entry) => entry.status === 'living')
        .map((entry) => entry.id),
      absoluteHour
    })
  };
  for (const systemId of Object.keys(simulation.systems)) {
    recomputeSystemFromSettlements(simulation, systemId, absoluteHour);
  }
  return simulation;
}
...
function worldEvent(
  ...
  draft: WorldEventDraft | null
): WorldEvent | null {
  return draft
    ? { ...draft, id: eventId(state, scheduled), atHour: scheduled.dueHour }
    : null;
}
...
function weightedAverage(
  ...
  return weight > 0
    ? values.reduce((sum, entry) => sum + entry.value * entry.weight, 0) / weight
    : fallback;
}

function resolveCivilizationCycle(
  state: SimulationState,
  ...
  const civilization = id
    ? context.galaxy.civilizations.find((entry) => entry.id === id)
    : undefined;
  if (!id || !current || !civilization || !current.alive) return null;

  const settlements = Object.values(state.settlements).filter(
    (entry) => entry.civilizationId === id && !entry.abandoned
  );
  const population = settlements.reduce((sum, entry) => sum + entry.population, 0);
  const stabilityTarget = weightedAverage(
    settlements.map((entry) => ({
      value: clamp(entry.security + entry.health - entry.unrest) / 2,
      weight: entry.population
    })),
    current.stability
  );
  const economyTarget = weightedAverage(
    settlements.map((entry) => ({
      value: (entry.infrastructure + (state.systems[entry.systemId]?.supply ?? 35)) / 2,
      weight: entry.population
    })),
    current.economy
  );
  const cohesionTarget = weightedAverage(
    settlements.map((entry) => ({ value: 100 - entry.unrest, weight: entry.population })),
    current.cohesion
  );
  const housingStress = weightedAverage(
    settlements.map((entry) => ({
      value: 100 - entry.housing + entry.unrest,
      weight: entry.population
    })),
    current.expansionPressure
  );
  const next: SimulationCivilizationState = {
    ...current,
    population,
    stability: clamp(
      current.stability + Math.round((stabilityTarget - current.stability) * 0.25)
    ),
    economy: clamp(
      current.economy + Math.round((economyTarget - current.economy) * 0.22)
    ),
    cohesion: clamp(
      current.cohesion + Math.round((cohesionTarget - current.cohesion) * 0.2)
    ),
    expansionPressure: clamp(
      current.expansionPressure +
        Math.round((housingStress - current.expansionPressure) * 0.18)
    ),
    alive: population > 0,
  ...
  if (!next.alive) {
    return worldEvent(state, event, {
      kind: 'disaster',
      title: `${civilization.name}: цивилизация прекратила существование`,
      summary:
        'Последние действующие поселения опустели. В галактике остались только инфраструктура, архивы и беженцы.',
      severity: 10,
      visibility: 'public',
      systemIds: civilization.controlledSystems,
      civilizationIds: [id],
      factionIds: context.factions
        .filter((entry) => entry.civilizationId === id)
        .map((entry) => entry.id),
      tags: ['simulation', 'civilization', 'extinction'],
      data: { population: 0 }
    });
  }

  const development = simulateCivilizationDevelopmentCycle(
    state,
    civilization,
    context,
    event.dueHour
  );
  if (development) return worldEvent(state, event, development);

  const stabilityDrop = current.stability - next.stability;
  if (stabilityDrop >= 8 || next.stability <= 24) {
    return worldEvent(state, event, {
      kind: 'politics',
      title: `${civilization.name}: системный кризис`,
      summary: `Стабильность ${next.stability}/100. Причина находится в состоянии реальных поселений, снабжения и населения.`,
      severity: 7,
      visibility: 'public',
      systemIds: settlements.map((entry) => entry.systemId).slice(0, 8),
      civilizationIds: [id],
      factionIds: context.factions
        .filter((entry) => entry.civilizationId === id)
        .map((entry) => entry.id),
      tags: ['simulation', 'civilization', 'causal'],
      data: { stability: next.stability, population }
    });
  }

  if (next.expansionPressure >= 75 && current.expansionPressure < 75) {
    return worldEvent(state, event, {
      kind: 'migration',
      title: `${civilization.name}: давление на внешние рубежи`,
      summary:
        'Перенаселение, жильё и локальные кризисы толкают частные и государственные экспедиции к новым территориям.',
      severity: 5,
      visibility: 'local',
      systemIds: settlements.map((entry) => entry.systemId).slice(0, 6),
      civilizationIds: [id],
      factionIds: context.factions
        .filter((entry) => entry.civilizationId === id)
        .map((entry) => entry.id),
      tags: ['simulation', 'civilization', 'expansion'],
      data: { expansionPressure: next.expansionPressure }
    });
  }
  return null;
}

function resolveFactionCycle(
  state: SimulationState,
  event: ScheduledWorldEvent,
  context: SimulationContext
): WorldEvent | null {
  ...
  const settlements = Object.values(state.settlements).filter(
    (entry) => entry.ownerFactionId === id && !entry.abandoned
  );
  ...
  const productive = settlements.length
    ? settlements.reduce(
        (sum, entry) => sum + entry.infrastructure - entry.unrest * 0.5,
        0
      ) / settlements.length
    : current.wealth - 8;
  const disruptedRoutes = routes.filter((entry) => entry.disrupted).length;
  const averageUnrest = settlements.length
    ? settlements.reduce((sum, entry) => sum + entry.unrest, 0) / settlements.length
    : 40;
  const wealthTarget = clamp(productive - disruptedRoutes * 5 + routes.length * 1.5);
  const tensionTarget = clamp(
    averageUnrest * 0.6 + faction.enemies.length * 15 + disruptedRoutes * 8
  );
  const next = {
    ...current,
    wealth: clamp(current.wealth + Math.round((wealthTarget - current.wealth) * 0.25)),
    influence: clamp(
      current.influence + Math.round((wealthTarget - current.influence) * 0.12)
    ),
    tension: clamp(
      current.tension + Math.round((tensionTarget - current.tension) * 0.2)
    ),
    lastUpdatedHour: event.dueHour
  };
  state.factions[id] = next;

  if (next.tension >= 78 && current.tension < 78) {
    return worldEvent(state, event, {
      kind: 'conflict',
      title: `${faction.name}: мобилизация`,
      ...
      factionIds: [id],
      tags: ['simulation', 'faction', 'causal'],
      data: { tension: next.tension, disruptedRoutes }
    });
  }
  if (current.wealth - next.wealth >= 8 || next.wealth <= 22) {
    return worldEvent(state, event, {
      kind: 'shortage',
      title: `${faction.name}: экономический спад`,
      summary: `Богатство ${next.wealth}/100. Производство поселений и торговые маршруты не покрывают потери.`,
      severity: 6,
      visibility: 'local',
      systemIds: settlements.map((entry) => entry.systemId).slice(0, 6),
      civilizationIds: faction.civilizationId ? [faction.civilizationId] : [],
      factionIds: [id],
      tags: ['simulation', 'faction', 'economy'],
      data: { wealth: next.wealth, settlements: settlements.length }
    });
  }
  return null;
}

function resolveSystemCycle(
  state: SimulationState,
  event: ScheduledWorldEvent,
  context: SimulationContext
): WorldEvent | null {
  ...
  if (next.supply <= 20 && before.supply > 20) {
    return worldEvent(state, event, {
      kind: 'shortage',
      title: `${system.name}: системный дефицит`,
      ...
      tags: ['simulation', 'system', 'settlements'],
      data: { supply: next.supply, population: next.population }
    });
  }
  if (next.migrationPressure >= 75 && before.migrationPressure < 75) {
    return worldEvent(state, event, {
      kind: 'migration',
      title: `${system.name}: массовый отток населения`,
      ...
      civilizationIds: system.civilizationIds,
      factionIds: system.factionId ? [system.factionId] : [],
      tags: ['simulation', 'system', 'migration'],
...
function resolveEcologyCycle(
  ...
  const system = context.galaxy.systems.find((entry) =>
    entry.planets.some((planet) => planet.id === planetId)
  );
  ...
  for (const settlement of settlements) {
    ...
    state.settlements[settlement.id] = {
      ...
      production: {
        ...
        food: Math.max(
          0,
          settlement.production.food * Math.max(0.45, biomassRatio)
        ),
        medicine: Math.max(
          0,
          settlement.production.medicine *
            Math.max(0.5, result.ecology.resources.medicinal / 50)
        )
      },
      health: clamp(
        settlement.health - (contamination >= 70 ? 8 : contamination >= 45 ? 3 : 0)
      ),
  ...
  return worldEvent(state, event, {
    ...
    summary: settlements.length
      ? `${result.event.summary} Изменение затронуло ${settlements.length} поселений.`
      : result.event.summary,
    severity: result.event.severity,
    visibility: result.event.visibility,
    systemIds: [system.id],
    civilizationIds: planet.civilizationId
      ? [planet.civilizationId]
      : system.civilizationIds,
    factionIds: system.factionId ? [system.factionId] : [],
...
function processScheduledEvent(
  ...
  if (event.kind === 'civilization-cycle') {
    return { event: resolveCivilizationCycle(state, event, context), scheduledEvents: [] };
  }
  if (event.kind === 'faction-cycle') {
    return { event: resolveFactionCycle(state, event, context), scheduledEvents: [] };
  }
  if (event.kind === 'system-cycle') {
    return { event: resolveSystemCycle(state, event, context), scheduledEvents: [] };
  }
  if (event.kind === 'ecology-cycle') {
    return { event: resolveEcologyCycle(state, event, context), scheduledEvents: [] };
  }
  if (event.kind === 'settlement-cycle') {
    const result = event.entityId
      ? simulateSettlementCycle(state, event.entityId, context, event.dueHour)
      : { event: null, abandoned: false };
    return { event: worldEvent(state, event, result.event), scheduledEvents: [] };
  }
  if (event.kind === 'trade-cycle') {
    const result = event.entityId
      ? simulateTradeRouteCycle(state, event.entityId, context, event.dueHour)
      : { event: null, moved: 0 };
    return { event: worldEvent(state, event, result.event), scheduledEvents: [] };
  }
  if (event.kind === 'migration-cycle') {
    const result = event.entityId
      ? simulateMigrationCycle(state, event.entityId, context, event.dueHour)
      : { event: null, scheduledEvents: [] };
    return {
      event: worldEvent(state, event, result.event),
...
function ensureSettlementSimulation(
  ...
  if (missing.length) {
    state = {
      ...state,
      scheduledEvents: [...state.scheduledEvents, ...missing].sort(
        (a, b) => a.dueHour - b.dueHour
      )
    };
  }
  return state;
...
export function advanceSimulation(
  ...
  const insertScheduled = (entry: ScheduledWorldEvent): void => {
    ...
    while (low < high) {
      ...
      if ((queue[middle]?.dueHour ?? Number.POSITIVE_INFINITY) <= entry.dueHour) {
        low = middle + 1;
      } else {
        high = middle;
      }
  ...
  while (queue[0] && queue[0].dueHour <= targetHour) {
    ...
    if (processed > 100_000) {
      throw new Error('Simulation advance exceeded the safe event budget');
    }
    ...
    if (result.event?.tags.includes('living-history-era')) {
      const continuitySchedules = missingSettlementSchedule({
        context,
        settlementIds: Object.keys(state.settlements),
        tradeRouteIds: Object.keys(state.tradeRoutes),
        civilizationIds: context.galaxy.civilizations
          ...
          .map((entry) => entry.id),
        existing: queue,
        absoluteHour: next.dueHour
      });
      for (const scheduled of continuitySchedules) insertScheduled(scheduled);
    }
    const settlementAbandoned =
      next.kind === 'settlement-cycle' &&
      next.entityId &&
      state.settlements[next.entityId]?.abandoned;
    const routeMissing =
      next.kind === 'trade-cycle' &&
      next.entityId &&
      !state.tradeRoutes[next.entityId];
    if (next.repeatHours && !settlementAbandoned && !routeMissing) {
      insertScheduled({ ...next, dueHour: next.dueHour + next.repeatHours });
    }
  }
...
export function adjustSystemEconomy(
  ...
  delta: Partial<
    Pick<
      SimulationSystemState,
      'supply' | 'prosperity' | 'security' | 'tradePressure' | 'migrationPressure'
    >
  >
): SimulationState {
...
export function upgradeSimulationEcosystems(
  ...
  const ecologyEvents: ScheduledWorldEvent[] = Object.keys(ecosystems)
    .map((planetId, index) => ({
      id: `ecology-cycle:${planetId}`,
      kind: 'ecology-cycle' as const,
      entityId: planetId,
  ...
  const base = {
    ...
    populationGroups: input.populationGroups ?? {},
    tradeRoutes: input.tradeRoutes ?? {},
    scheduledEvents: [...input.scheduledEvents, ...ecologyEvents].sort(
      (a, b) => a.dueHour - b.dueHour
    )
  } as SimulationState;
...
export function adjustEcosystem(
  ...
  delta: Partial<
    Pick<
      import('../ecology/types').PlanetEcologyState,
      'biomass' | 'biodiversity' | 'resilience' | 'contamination' | 'climateStability'
    >
  >
): SimulationState {
