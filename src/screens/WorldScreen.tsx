...
export function WorldScreen({ chrome }: { chrome: React.ReactNode }) {
  ...
  if (compact) {
    return <div className="game-shell">{chrome}<main className="mobile-data-screen world-mobile">
      <header className="mobile-screen-header"><div><span className="eyebrow">ПОДТВЕРЖДЁННАЯ СВЯЗЬ</span><h1>Живой мир</h1></div><b>{visibleThreads.length + visibleSettlements.length}</b></header>
      {!selected && !selectedSettlement && <nav className="mobile-segmented three" aria-label="Раздел живого мира">
      ...
      </section> : mobileTab === 'threads' ? <section className="mobile-list">{visibleThreads.length ? visibleThreads.map((thread) => <button className="mobile-list-row" key={thread.id} onClick={() => setSelectedId(thread.id)}><span>{categoryLabel[thread.category]} · {thread.status}</span><b>{thread.title}</b><i style={{ width: `${thread.progress}%` }}/></button>) : <div className="mobile-empty"><b>Связь молчит</b><p>Посети поселение или установи контакт.</p></div>}</section>
