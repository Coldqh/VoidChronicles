...
export function WorldScreen({ chrome }: { chrome: React.ReactNode }) {
  ...
  const visibleNeeds = useMemo(() => worldNeeds.filter((need) => knownSystemIds.has(need.targetSystemId) || need.civilizationIds.some((id) => knownCivilizationIds.has(id))), [worldNeeds, knownSystemIds, knownCivilizationIds]);
