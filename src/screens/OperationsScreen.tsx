...
type Tab = 'requests' | 'active' | 'intel';
...
export function OperationsScreen({ chrome }: { chrome: ReactNode }) {
  ...
  return <div className="game-shell">{chrome}<main className="v35-operations">
    <div className="v35-operations-ambient" aria-hidden="true"><i/><i/><i/></div>
    <header className="v35-screen-hero">
      <div><span className="eyebrow">ЦЕНТР УПРАВЛЕНИЯ ОПЕРАЦИЯМИ</span><h1>Решения меняют мир</h1><p>Здесь нет одинаковых поручений. Каждый запрос вырос из войны, дефицита, открытия или чужого страха.</p></div>
      <div className="v35-hero-score"><span>ТРЕБУЮТ РЕШЕНИЯ</span><b>{requests.length + active.length}</b><small>готовность экипажа {readiness}%</small></div>
    </header>

    <nav className="v35-segmented">
    ...
    {tab === 'active' && <section className="v35-active-board">
      {active.map((objective) => {
        ...
        return <article key={objective.id} className={`v35-active-mission category-${operation.category}`}>
          <header><div><span className="eyebrow">{operationLabels[operation.category]} · {operation.issuerName}</span><h2>{objective.title}</h2><p>{objective.description}</p></div><div className="v35-progress-orb" style={{ '--progress': `${objective.progress * 3.6}deg` } as React.CSSProperties}><b>{formatInteger(objective.progress)}%</b></div></header>
