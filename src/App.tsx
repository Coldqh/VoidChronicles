...
function CommandDeckScreen() {
  ...
  const opportunities = [
    ...
    activeContracts[0] ? { title: activeContracts[0].title, text: `Цель: ${galaxy.systems.find((system) => system.id === activeContracts[0].targetSystemId)?.name ?? 'неизвестная система'}. Срок: ${activeContracts[0].deadlineYear}.`, action: () => store.setScreen('contracts'), label: 'Открыть контракт' } : null,
    !localHubs.length ? { title: 'Система без безопасного порта', text: 'Нужно сканировать планеты, искать сигналы или уходить дальше.', action: () => store.setScreen('system'), label: 'Открыть систему' } : null
  ].filter(Boolean) as { title: string; text: string; action(): void; label: string }[];
