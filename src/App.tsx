...
function StoryScenePopup() {
  ...
  const scene = store.storyScenes.find((entry) => entry.id === store.activeStorySceneId && entry.status === 'available');
  useEffect(() => { setMessage(''); }, [scene?.id]);
  if (!scene) return null;
  const system = store.galaxy?.systems.find((entry) => entry.id === scene.systemId);
  return <div className="story-popup-backdrop" role="presentation">
    <section className={`story-popup scene-${scene.category}`} role="dialog" aria-modal="true" aria-label={scene.title}>
      <header><div><span className="eyebrow">{scene.category.toUpperCase()} · {system?.name ?? 'НЕИЗВЕСТНЫЙ ИСТОЧНИК'}</span><h2>{scene.title}</h2></div><button className="icon-button" aria-label="Закрыть" onClick={store.closeStoryScene}>×</button></header>
      <p className="story-popup-summary">{scene.summary}</p><p className="story-popup-body">{scene.body}</p>
      <div className="story-popup-choices">{scene.choices.map((choice) => <button key={choice.id} className={`choice-risk-${choice.risk}`} disabled={Boolean(store.busyAction)} onClick={async () => { const result = await store.resolveStoryScene(scene.id, choice.id); setMessage(result.message); }}><b>{choice.label}</b><span>{choice.summary}</span></button>)}</div>
      {message && <p className="story-popup-message">{message}</p>}
    </section>
  </div>;
}
...
function CommandDeckScreen() {
  ...
  const primary = tutorialActive
    ...
    : !current.scanned
      ...
      : activeScenes[0]
        ? { title: activeScenes[0].title, text: activeScenes[0].summary, label: 'Ответить', action: () => store.openStoryScene(activeScenes[0]!.id), tutorial: false }
        : currentPoints[0]
  ...
  if (compact) {
    return <div className="game-shell"><AppChrome/><main className="mobile-dashboard">
      ...
      <nav className="mobile-action-grid" aria-label="Быстрые действия">
        ...
        <button onClick={() => activeScenes[0] ? store.openStoryScene(activeScenes[0].id) : store.setScreen('operations')}><i>⚠</i><b>{activeScenes.length ? 'Сигнал' : 'Угрозы'}</b></button>
      </nav>
...
function SystemScreen() {
  ...
  const ecology = planet?.scanLevel && planet.scanLevel >= 2 ? store.simulation?.ecosystems[planet.id] : undefined;
  ...
  if (compact) {
    return <div className="game-shell"><AppChrome/><main className="mobile-map-screen system-mobile-map">
      ...
      {sheetOpen && planet && <><button className="mobile-window-scrim" aria-label="Закрыть окно планеты" onClick={() => setSheetOpen(false)}/><aside className="mobile-map-window system-object-window" role="dialog" aria-label={`Объект: ${planet.name}`}>
        ...
        <div className="mobile-window-body"><p>{planet.scanLevel ? `Угроза: ${planet.danger}` : 'Характеристики не подтверждены.'}</p>{report && <small>{report.summary}</small>}{ecology && <details className="mobile-window-details ecology-summary"><summary>Экосистема · {formatInteger(ecology.biodiversity)}/100</summary><div className="ecology-metrics"><span>Биомасса <b>{formatInteger(ecology.biomass)}</b></span><span>Климат <b>{formatInteger(ecology.climateStability)}</b></span><span>Загрязнение <b>{formatInteger(ecology.contamination)}</b></span></div><p>{ecology.biomes.slice(0, 3).map((entry) => entry.name).join(' · ')}</p><small>Видов: {ecology.species.filter((entry) => entry.status !== 'extinct').length} · патогенов: {ecology.pathogens.filter((entry) => entry.active).length}</small><div className="ecology-species-list">{ecology.species.filter((entry) => entry.status !== 'extinct').sort((a, b) => b.abundance - a.abundance).slice(0, 3).map((entry) => <span key={entry.id}><b>{entry.name}</b><small>{entry.trophicLevel} · {entry.status}</small></span>)}</div></details>}{planetPoints.length > 0 && <div className="mobile-poi-list">{planetPoints.map((entry) => { const state = store.locationStates.find((location) => location.pointOfInterestId === entry.id); const surface = entry.access === 'surface'; return <button disabled={!surface && entry.status === 'resolved'} data-tutorial={entry.id === store.tutorial.targetPointOfInterestId ? 'open-expedition' : undefined} key={entry.id} onClick={async () => { if (surface) { setPoint(entry); if (entry.id === store.tutorial.targetPointOfInterestId) void store.advanceTutorial(4); } else setNotice((await store.investigatePoint(entry.id)).message); }}><b>{entry.name}</b><span>{surface ? 'Высадка' : entry.access === 'orbital' ? 'Орбитальный анализ' : 'Дистанционный анализ'} · {state ? `визитов ${state.visitCount}` : entry.danger}</span></button>; })}</div>}{system.scanned && (localHubs.length > 0 || systemCivilizations.length > 0) && <details className="mobile-window-details"><summary>Связь и поселения · {localHubs.length + systemCivilizations.length}</summary>{systemCivilizations.map((civilization) => <button key={civilization.id} onClick={async () => setNotice((await store.attemptFirstContact(civilization.id)).message)}>{civilization.name} · связь</button>)}{localHubs.map((hub) => <button key={hub.id} onClick={() => void store.dockAtHub(hub.id)}>{hub.name}</button>)}</details>}</div>
  ...
  return <div className="game-shell"><AppChrome/><main className="map-screen system-screen-separated system-screen-clean">
    ...
    <aside className="system-object-panel">{notice && <p className="notice-inline">{notice}</p>}{planet ? <><div className={`planet-orb planet-${planet.type}`}><span/></div><h2>{planet.scanLevel ? planet.name : 'НЕИЗВЕСТНЫЙ ОБЪЕКТ'}</h2><p>{planet.scanLevel ? `${planet.type} · угроза ${planet.danger}` : 'Нет подтверждённых характеристик.'}</p><button data-tutorial={planet.id === store.tutorial.targetPlanetId ? 'detail-scan' : undefined} disabled={!system.scanned || Boolean(store.busyAction)} onClick={() => void scanPlanet()}>Детальный скан</button>{report && <article className="scan-report"><span>ДОСТОВЕРНОСТЬ {Math.round(report.confidence)}%</span><p>{report.summary}</p></article>}{ecology && <article className="scan-report ecology-report"><span>ЭКОСИСТЕМА · ЦИКЛ {formatInteger(ecology.cycle)}</span><div className="ecology-metrics"><span>Биомасса <b>{formatInteger(ecology.biomass)}</b></span><span>Разнообразие <b>{formatInteger(ecology.biodiversity)}</b></span><span>Климат <b>{formatInteger(ecology.climateStability)}</b></span></div><p>{ecology.biomes.slice(0, 4).map((entry) => entry.name).join(' · ')}</p><small>Живых видов: {ecology.species.filter((entry) => entry.status !== 'extinct').length} · активных патогенов: {ecology.pathogens.filter((entry) => entry.active).length}</small><div className="ecology-species-list">{ecology.species.filter((entry) => entry.status !== 'extinct').sort((a, b) => b.abundance - a.abundance).slice(0, 5).map((entry) => <span key={entry.id}><b>{entry.name}</b><small>{entry.trophicLevel} · численность {formatInteger(entry.abundance)}</small></span>)}</div></article>}<div className="poi-list">{planetPoints.length ? planetPoints.map((entry) => { const state = store.locationStates.find((location) => location.pointOfInterestId === entry.id); const surface = entry.access === 'surface'; return <article className={`poi-card poi-${entry.status}`} key={entry.id}><div><span className="eyebrow">{entry.type} · {entry.danger} · {entry.access}</span><h3>{entry.name}</h3><p>{entry.publicSummary}</p>{state && <small>Визитов {state.visitCount} · данных забрано {state.collectedEvidenceKeys.length}</small>}</div><button disabled={!surface && entry.status === 'resolved'} data-tutorial={entry.id === store.tutorial.targetPointOfInterestId ? 'open-expedition' : undefined} onClick={async () => { if (surface) { setPoint(entry); if (entry.id === store.tutorial.targetPointOfInterestId) void store.advanceTutorial(4); } else setNotice((await store.investigatePoint(entry.id)).message); }}>{surface ? 'Высадка' : entry.access === 'orbital' ? 'Орбитальный анализ' : 'Дистанционный анализ'}</button></article>; }) : <p className="empty-state">{planet.scanLevel && planet.scanLevel >= 2 ? 'Значимых сигналов нет.' : 'Нужен детальный скан.'}</p>}</div></> : <div className="empty-selection"><b>Выбери орбитальный объект</b><p>До сканирования он останется без имени и характеристик.</p></div>}</aside>
    {expedition}
  </main></div>;
}
...
function HubScreen() {
  ...
  const market = generateMarket(hub, store.gameYear, store.simulation?.systems[hub.systemId]);
