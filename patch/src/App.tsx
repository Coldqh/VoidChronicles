...
function MainMenu() {
  ...
  return <main className="menu-screen">
    ...
    <section className="menu-panel">
      ...
      <footer>v0.1.1 · STABILITY & SAVE INTEGRITY</footer>
    </section>
  </main>;
}
...
function GalaxyScreen() {
  ...
  const canTravel = selected.id !== current.id && direct && jumpDistance <= store.ship.jumpRange && !store.busyAction && store.ship.hull > 0;
    ...
    <div className="galaxy-layout">
      ...
      <aside className="system-panel">
        ...
        <div className="action-row">
          {selected.id === current.id ? <button className="primary-button" disabled={Boolean(store.busyAction)} onClick={() => store.scanSystem(selected.id)}>{selected.scanned ? 'Повторный скан' : 'Сканировать'}</button> : <button className="primary-button" disabled={!canTravel} onClick={doTravel}>Прыжок · {Math.max(7, Math.ceil(jumpDistance / 14))} топлива</button>}
        </div>
        ...
        <div className="planet-list">
          {selected.planets.map((planet) => <article className={`planet-card ${!selected.scanned ? 'locked' : ''}`} key={planet.id}>
            ...
            {selected.id === current.id && selected.scanned && planet.type !== 'gas' && <button disabled={Boolean(store.busyAction)} onClick={() => setExpeditionPlanet(planet)}>Высадка</button>}
          </article>)}
        </div>
      </aside>
